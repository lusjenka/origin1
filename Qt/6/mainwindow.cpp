...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        auto race1_ = [this] {concurRace1->DoWork(&number,ui->rb_mutexOn->isChecked(),ui->sb_initNum->value());  };
        auto race2_ = [this] {concurRace2->DoWork(&number,ui->rb_mutexOn->isChecked(),ui->sb_initNum->value());  };
        QFuture<void> future;
        future = QtConcurrent::run(race1_).then(race2_);
